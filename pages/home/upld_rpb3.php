...
if(isset($_POST['Upload_Peng'])){
    ...
    // Import data excel mulai baris ke-2 (karena tabel xlsx ada header pada baris 1)
    for ($row = 2; $row <= $highestRow; ++$row) {
        ...
        $query_insert = 'INSERT INTO develop."RPB_REKON" (
            "ID_IHLD", "PROGRAM", "NILAI_REALISASI", "TAHUN", "KET_MITRA", 
            "PROJECT_DESC", "BULAN", "PROJECT_STATUS", 
            "TERITORY", "STATUS_SO", "AREA", "BUDGET_SOURCE", "TIMESTAMP"
        ) VALUES (
            \''.$id_ihld.'\', \''.$program.'\', '.$nilai_realisasi.', \''.$tahun.'\', \''.$ketmitra.'\', 
            \''.$project_desc.'\', \''.$bulan.'\', \''.$project_status.'\', \''.$teritory.'\', 
            \''.$status_so.'\', \''.$area.'\', \''.$budget.'\', \''.$tanggal_sekarang.'\'
        );';
    ...
    ?>
        ...
        setTimeout("location.href='?pages=upld_rpb3'", 1000);
        </script>
    <?php
    } else {
...
if (isset($_POST['Batal'])) {
    echo "<script>document.location.href='?pages=upld_rpb3';</script>";
}
?>
...
<h1><i class="fa fa-upload" aria-hidden="true"></i> Halaman Upload RPB DUPLIKAT</h1>
</div>
</div>
</div>
<!-- ==================================> Halaman Upload LOP <============================== -->
